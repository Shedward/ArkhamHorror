...
struct ExpressionParsersRepository<Context> {
	...
	func parser<Result>(head: String, returnType: Result.Type) -> AnyExpressionParser<Context, Result>? {
		let descriptor = ParserDescriptor(head: head, returnType: returnType)
		return parsers[descriptor] as? AnyExpressionParser<Context, Result>
	}

	func documentationSuggestion(for headPrefix: String) -> [ExpressionDoc] {
		parsers
