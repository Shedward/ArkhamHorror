...
final class MapDropDelegate: DropDelegate {
	...
	func performDrop(info: DropInfo) -> Bool {
        guard let fileProvider = info.itemProviders(for: [.fileURL, .yaml]).first else {
			return false
		}
        ...
        fileProvider.loadDataRepresentation(forTypeIdentifier: UTType.yaml.identifier) { data, error in
            let loadingResult = self.toResult(
                data: data,
                error: error,
                mapError: Error.itemProviderFailedToLoadObject
    ...
    private func loadMap(from data: Data) -> Result<Map, Error> {
        let map: Map
        do {
            map = try Map(data: data)
        } catch {
