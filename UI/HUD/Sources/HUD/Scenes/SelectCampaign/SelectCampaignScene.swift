...
final class SelectCampaignScene: Scene<SelectCampaignViewModel>, SelectCampaignSceneProtocol {
    ...
    private var collection: Collection<CampaignInfo, Button>?

    override func setup() {
        super.setup()

        let cellSize = CGSize(width: 128, height: 256)
        let cellProvider = FnCollectionCellProvider { (info: CampaignInfo) -> Button in
            Button(texture: SKTexture(imageNamed: info.name)) {
                Logger().debug("Did tap button for \(info.name)")
            }
        }
